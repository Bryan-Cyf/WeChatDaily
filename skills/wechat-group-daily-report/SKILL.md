...
> ⚠️ **铁律：一步都不能少。**
> 即使用户在首条消息里已经给了群名、日期、是否解密，**也不能跳过 Step 1–3 的确认**。
> 用户提供的信息只作为**预填默认值**，仍必须：
> 1. 该拉的数据照拉（如 Step 1 的群列表）；
> 2. 用 `AskQuestion` 把预填结果回显给用户**逐项确认**；
> 3. 得到用户明确确认后才进入下一步。
> 禁止以「用户已经说了」为由直接跳到 Step 4。

复制进度清单：

```
- [ ] Step 1 确认群名（先拉全量群列表比对，再确认）
- [ ] Step 2 确认日期
- [ ] Step 3 确认是否刷新解密
- [ ] Step 4 导出聊天记录 MD
...
### Step 1：确认群名（**必做，不可凭用户输入直接定群**）

无论用户是否已经给了群名，都**必须**先拉取当前全部群再比对确认：

1. **拉全量群列表**（需已 decrypt；若报错见下方说明）：
...
2. **比对**：
   - 用户**已给群名** → 拿用户给的片段在群列表里做匹配：
     - 唯一精确/模糊命中 → 作为预填项，仍需第 3 步回显确认；
     - 命中多个 → 列出候选让用户选；
     - 零命中 → 告知未找到，展示相近群名供用户重选。
   - 用户**未给群名** → 直接展示群列表（或活跃群）供选择。
3. **回显确认**：用 `AskQuestion` 把「我将分析的群」逐项列给用户，得到明确确认后才进入 Step 2。

说明：

- 支持多个群：逗号分隔，与 `groups` 命令显示名**完全一致**
- 群名含逗号时用引号包裹整个 `--groups` 参数
- 若 `groups` 因尚未解密而报错/为空 → 先与用户确认走 Step 3 解密，再回到 Step 1 拉列表

**示例提问（用户已给群名时）**：

> 你提到的「{用户片段}」匹配到群「{命中群名}」，确认就分析这个群吗？

**示例提问（用户未给群名时）**：

> 当前可选群如下，要生成哪几个群的日报？（可多选）

### Step 2：确认日期（**必做**）

无论用户是否已给日期，都用 `AskQuestion` 回显确认：

- **默认**：当天，`YYYY-MM-DD`（本地时区）
- 用户已给日期 → 作为预填项回显确认（如校验格式是否合法）
- 用户未指定 → 预填今天 `{today}`

**示例提问**：

> 分析哪一天的数据？{用户已给则填该日期，否则填今天 `{today}`}，确认吗？

### Step 3：确认是否拉取最新数据（**必做**）

无论用户是否已表态，都用 `AskQuestion` 回显确认（把用户倾向设为预填默认项）：
